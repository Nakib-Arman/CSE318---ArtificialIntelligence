    ...
    for (int i=0;i<grid.size();i++) {
        for (int j=0; j<grid[0].size(); j++) {
            if (grid[i][j].get_color() == ai_color) {
                ...
                cout<<i<<" ai "<<j<<" "<<ai_count<<endl;
            }
            else if (grid[i][j].get_color() != 'W') {
                cout<<grid[i][j].get_orb_num()<<" "<<grid[i][j].get_critical_mass()<<endl;;
                user_count += static_cast<float>(grid[i][j].get_orb_num() / grid[i][j].get_critical_mass());
                cout<<i<<" user "<<j<<" "<<user_count<<endl;
            }
        }
    }
    cout<<ai_count<<" "<<user_count<<endl;
    return ai_count - user_count;
}
...
int main()
{
    ...
    int a = heuristic5(grid,ai_color);
    cout<<a<<endl;
}