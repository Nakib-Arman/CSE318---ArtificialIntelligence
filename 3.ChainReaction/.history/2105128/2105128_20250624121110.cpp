    ...
    for (int i=0;i<grid.size();i++) {
        for(int j =0; j<grid[0].size();j++) {
            if (i!=0 && j!=0 && i!=grid.size()-1 && j!=grid[0].size()-1) {
                if(grid[i][j].get_color() == ai_color) ai_count++;
                else if(grid[i][j].get_color() != 'W') user_count++;
            }
            if (grid[i][j].get_color() == ai_color) {
                ai_count+=2;
            }
...
float heuristic4(vector<vector<Cell>>& grid, char ai_color) {
    ...
    for (int i=0;i<grid.size();i++) {
        for(int j =0; j<grid[0].size();j++) {
            if (i!=0 && j!=0 && i!=grid.size()-1 && j!=grid[0].size()-1) {
                if(grid[i][j].get_color() == ai_color) ai_count+= grid[i][j].get_orb_num();
                else if(grid[i][j].get_color() != 'W') user_count+= grid[i][j].get_orb_num();;
            }
            if (grid[i][j].get_color() == ai_color) {
                ai_count += 2*grid[i][j].get_orb_num();
            }
            else if (grid[i][j].get_color() != 'W') {
                user_count += 2*grid[i][j].get_orb_num();
            }
        }
    }
    return static_cast<float>(ai_count - user_count);
...
pair<int,int> heuristic5(vector<vector<Cell>>& grid, char ai_color,int x,int y) {
    int m,n;
    m = grid.size();
    n = grid[0].size();
    cout<<x<<" "<<y<<endl;
    cout<<m-x<<" "<<n
    return make_pair(m-x-1,n-y-1);
}
...
pair<int,int> getBestMove(vector<vector<Cell>>& grid,char ai_color,char user_color,int depth,int x,int y) {
    ...
    pair <int,int> move = heuristic5(grid,ai_color,x,y);
    return move;
}

void aiMove(vector<vector<Cell>>& grid,char ai_color,char user_color,int depth,float (*func)(vector<vector<Cell>>&,char),int x,int y) {
    pair<int,int> best_move = getBestMove(grid,ai_color,user_color,depth,x,y);
    inc_cell_orbs(grid,ai_color,best_move.first,best_move.second);
}

void randomMove(vector<vector<Cell>>& grid,char random_color) {
    vector<pair<int,int>> valid_moves;
    for(int i=0;i<grid.size();i++) {
        for(int j=0;j<grid[0].size();j++) {
            if(!isValidMove(grid,random_color,i,j)) continue;
    ...
    int index = dist(gen);
    int x = valid_moves[index].first;
    int y = valid_moves[index].second;
    inc_cell_orbs(grid,random_color,x,y);
}


int main(int argc, char *argv[]) {
    int m = stoi(argv[1]),n = stoi(argv[2]);
    int user_move_x,user_move_y;
    // cin>>m>>n;
    ...
    if (move == 0) {
        ...
        user_move_x = stoi(argv[3]);
        user_move_y = stoi(argv[4]);
        ...
        aiMove(grid,ai_color,user_color,stoi(argv[4]),heuristic1,user_move_x,user_move_y);
    ...
    //         aiMove(grid,ai_color,user_color,stoi(argv[4]),heuristic2);
    //     }
    //     if(stoi(argv[3]) == 2) {
    //         aiMove(grid,ai_color,user_color,stoi(argv[4]),heuristic3);
    //     }
    //     if(stoi(argv[3]) == 3) {
    //         aiMove(grid,ai_color,user_color,stoi(argv[4]),heuristic4);
    //     }
    //     if(stoi(argv[3]) == 4) {
    //         aiMove(grid,ai_color,user_color,stoi(argv[4]),heuristic5);
    //     }
    //     file2 << "Random Move:\n";
    //     for (int i=0;i<m;i++){
    //         for (int j=0;j<n;j++){
