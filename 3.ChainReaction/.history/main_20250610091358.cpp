...
int minimax(vector<vector<Cell>>& grid,char color,int depth,int alpha,int beta,char user_color,char ai_color) {
    ...
    if(color == ai_color) {
        ...
        for(int i=0;i<grid.size();i++){
            for(int j=0;j<grid[0].size();j++){
                if(!isValidMove(grid,user_color,i,j)) continue;
                vector<vector<Cell>> temp = grid;
                inc_cell_orbs(temp,user_color,i,j);
        ...
        for(int i=0;i<grid.size();i++){
            for(int j=0;j<grid[0].size();j++){
                ...
                int value = minimax (temp,ai,depth-1,alpha,beta,user_color,ai_color);
...
void aiMove(vector<vector<Cell>>& grid,char ai_color,char user_color) {
