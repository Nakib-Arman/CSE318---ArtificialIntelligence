...
float minimax(vector<vector<Cell>>& grid,char color,int depth,float alpha,float beta,char user_color,char ai_color) {
    // for(int i=0;i<grid.size();i++){
    //     for(int j=0;j<grid[0].size();j++){
    //         grid[i][j].printCell();
    //         cout<<" ";
...
int main(int argc, char *argv[]) {
    ...
    if (move == 0) {
        ...
        else aiMove(grid,user_color,ai_color,DEPTH);
