...
int evaluate(vector<vector<Cell>>& grid, char ai_color) {
    int ai_orbs = 0, user_orbs = 0;
    int ai_stable = 0, user_stable = 0;
    int ai_critical = 0, user_critical = 0;
    int ai_corners = 0, user_corners = 0;
    int ai_edges = 0, user_edges = 0;
    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            Cell cell = grid[i][j];
            char c = cell.get_color();
            int orbs = cell.get_orb_num();
            int critical = cell.get_critical_mass();

            if (c == ai_color) {
                ai_orbs += orbs;
                if (critical - orbs >= 2) ai_stable++;
                if (critical - orbs <= 1) ai_critical++;
                if ((i == 0 || i == rows-1) && (j == 0 || j == cols-1)) ai_corners++;
                else if (i == 0 || j == 0 || i == rows-1 || j == cols-1) ai_edges++;
            }
            else if (c != 'W') {
                user_orbs += orbs;
                if (critical - orbs >= 2) user_stable++;
                if (critical - orbs <= 1) user_critical++;
                if ((i == 0 || i == rows-1) && (j == 0 || j == cols-1)) user_corners++;
                else if (i == 0 || j == 0 || i == rows-1 || j == cols-1) user_edges++;
            }
        }
    }

    int score = 10 * (ai_orbs - user_orbs)
              + 6 * (ai_stable - user_stable)
              + 4 * (ai_critical - user_critical)
              + 3 * (ai_corners - user_corners)
              + 2 * (ai_edges - user_edges);
...
int minimax(vector<vector<Cell>>& grid,char color,int depth,int alpha,int beta,char user_color,char ai_color) {
    if(depth == 0 || foundWinner(grid) != 'W') return evaluate(grid,ai_color);
    if(color == ai_color) {
...
pair<int,pair<int,int>> getBestMove(vector<vector<Cell>>& grid,char ai_color,char user_color,int depth) {
    ...
    for(int i=0;i<grid.size();i++){
        for(int j=0;j<grid[0].size();j++){
            ...
            int value = minimax(temp,ai_color,depth,INT_MIN,INT_MAX,user_color,ai_color);
            if (value>best_value) {
                best_value = value;
...
int main() {
    ...
    while(1){
        ...
        // aiMove(grid,user_color,ai_color,1);
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                grid[i][j].printCell();
                cout<<" ";
        ...
        if(winner != 'W'){
            ...
            else cout<<"AI is the winner"<<endl;
            break;
        }
        cout<<endl<<endl;
