...
float minimax(vector<vector<Cell>>& grid,char color,int depth,float alpha,float beta,char user_color,char ai_color,int (*func)(vector<vector<Cell>>*,char)) {
    // for(int i=0;i<grid.size();i++){
    //     for(int j=0;j<grid[0].size();j++){
    //         grid[i][j].printCell();
    //         cout<<" ";
    ...
    if(depth == 0 || foundWinner(grid) != 'W'){
        return heuristic2(grid,ai_color);
    }
    if(color == ai_color) {
        ...
        for(int i=0;i<grid.size();i++){
            for(int j=0;j<grid[0].size();j++){
                ...
                float value = minimax (temp,user_color,depth-1,alpha,beta,user_color,ai_color);
        ...
        for(int i=0;i<grid.size();i++){
            for(int j=0;j<grid[0].size();j++){
                ...
                vector<vector<Cell>> temp = grid;
                inc_cell_orbs(temp,color,i,j);
                float value = minimax (temp,ai_color,depth-1,alpha,beta,user_color,ai_color);
...
int main(int argc, char *argv[]) {
    ...
    if (move == 0) {
        ...
        if(move == 1 || move == 3) aiMove(grid,ai_color,user_color,DEPTH);
        else aiMove(grid,user_color,ai_color,0);
