...
def ai_vs_ai_menu():
    ...
    option_height = 40
    dropdowns = {"ai1": False, "depth1": False, "ai2": False, "depth2": False}
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if ai1_rect.collidepoint(mx, my):
                    dropdowns = {"ai1": not dropdowns["ai1"], "depth1": False, "ai2": False, "depth2": False}

                elif ai2_rect.collidepoint(mx, my):
                    dropdowns = {"ai1": False, "depth1": False, "ai2": not dropdowns["ai2"], "depth2": False}

                elif depth1_rect.collidepoint(mx, my):
                    dropdowns = {"ai1": False, "depth1": not dropdowns["depth1"], "ai2": False, "depth2": False}

                elif depth2_rect.collidepoint(mx, my):
                    dropdowns = {"ai1": False, "depth1": False, "ai2": False, "depth2": not dropdowns["depth2"]}
                    ...
                    for i, option in enumerate(depths):
                        ...
                        if option_rect.collidepoint(mx, my):
                            ...
                            dropdowns["depth1"] = False
                            break
                    else:
                        dropdowns["depth1"] = False

                elif dropdowns["depth2"]:
                    for i, option in enumerate(depths):
                    ...
                    depth1 = int(depth1_option)
                    depth2 = int(depth2_option)
                    play_ai_vs_ai()

                elif back_button.collidepoint(event.pos):
                    return

        def draw_dropdown(label, rect, option, options, open_flag):
            screen.blit(font.render(label, True, (255, 255, 255)), (rect.x - 150, rect.y + 5))
            pygame.draw.rect(screen, (200, 200, 200), rect)
            pygame.draw.rect(screen, (0, 0, 0), rect, 2)
            screen.blit(font.render(option, True, (0, 0, 0)), (rect.x + 10, rect.y + 5))
            if open_flag:
                for i, opt in enumerate(options):
                    opt_rect = pygame.Rect(rect.x, rect.y + (i+1)*option_height, rect.width, option_height)
                    pygame.draw.rect(screen, (255, 255, 255), opt_rect)
                    pygame.draw.rect(screen, (0, 0, 0), opt_rect, 1)
                    screen.blit(font.render(opt, True, (0, 0, 0)), (opt_rect.x + 10, opt_rect.y + 5))

        draw_dropdown("Heuristic:", ai1_rect, ai1_option, heuristics, dropdowns["ai1"])
        draw_dropdown("Heuristic:", ai2_rect, ai2_option, heuristics, dropdowns["ai2"])
        draw_dropdown("Depth:", depth1_rect, depth1_option, depths, dropdowns["depth1"])
        draw_dropdown("Depth:", depth2_rect, depth2_option, depths, dropdowns["depth2"])

        pygame.draw.rect(screen, (0, 200, 0), play_button_rect)
        pygame.draw.rect(screen, (0, 0, 0), play_button_rect, 2)
        play_text = font.render("Play", True, (255, 255, 255))
        screen.blit(play_text, (play_button_rect.x + 35, play_button_rect.y + 10))
