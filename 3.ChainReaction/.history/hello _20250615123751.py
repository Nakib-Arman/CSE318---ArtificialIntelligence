...
def settings_menu():
    global rows, cols, counts, colors, borders, orb_color1, orb_color2, orb_border1, orb_border2, cell_x, cell_y, cell_surface
    ...
    apply_button = pygame.Rect(popup_x + 200, popup_y + 320, 200, 50)

    input_boxes = {
        "Rows": [pygame.Rect(popup_x + 150, popup_y + 80, 100, 40), str(rows)],
        "Cols": [pygame.Rect(popup_x + 150, popup_y + 140, 100, 40), str(cols)],
        "Red":  [pygame.Rect(popup_x + 150, popup_y + 200, 100, 40), str(orb_color1)],
        "Blue": [pygame.Rect(popup_x + 150, popup_y + 260, 100, 40), str(orb_color2)],
    }
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if apply_button.collidepoint(event.pos):
                    try:
                        rows = int(input_boxes["Rows"][1])
                        cols = int(input_boxes["Cols"][1])
                        orb_color1 = eval(input_boxes["Red"][1])
                        orb_color2 = eval(input_boxes["Blue"][1])
                        orb_border1 = tuple(max(0, c - 150) for c in orb_color1)
                        orb_border2 = tuple(max(0, c - 150) for c in orb_color2)
                        cell_x, cell_y = (screen_x-700) // cols, (screen_y-100) // rows
                        cell_surface = pygame.Surface((cell_x,cell_y),pygame.SRCALPHA)
                        reset_game_state()
                        return
                ...
                for key, (rect, _) in input_boxes.items():
                    if rect.collidepoint(event.pos):
                        active_box = key

            elif event.type == pygame.KEYDOWN and active_box:
                if event.key == pygame.K_BACKSPACE:
                    input_boxes[active_box][1] = input_boxes[active_box][1][:-1]
                else:
                    input_boxes[active_box][1] += event.unicode

        screen.blit(background, (0, 0))
        pygame.draw.rect(screen, (245, 245, 245), popup_rect, border_radius=10)
        pygame.draw.rect(screen, (255, 255, 255), popup_rect, 3, border_radius=10)

        title = title_font.render("Settings", True, (0, 0, 0))
        screen.blit(title, (popup_x + 220, popup_y + 20))

        labels = ["Rows", "Cols", "Red", "Blue"]
        for idx, key in enumerate(labels):
            text = font.render(f"{key}:", True, (0, 0, 0))
            screen.blit(text, (popup_x + 50, popup_y + 80 + idx * 60))
            rect, value = input_boxes[key]
            pygame.draw.rect(screen, (255, 255, 255), rect)
            pygame.draw.rect(screen, (0, 0, 0), rect, 2)
            value_surf = font.render(value, True, (0, 0, 0))
            screen.blit(value_surf, (rect.x + 5, rect.y + 5))

        pygame.draw.rect(screen, (0, 128, 0), apply_button)
        apply_text = font.render("Apply", True, (255, 255, 255))
        screen.blit(apply_text, apply_text.get_rect(center=apply_button.center))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            update_grid()
            if winner != 'none':
                sttings_button = load_ui()
                game_over()
                counts = [[0 for _ in range(cols)] for _ in range(rows)]
                colors = [['W' for _ in range(cols)] for _ in range(rows)]
                borders = [['W' for _ in range(cols)] for _ in range(rows)]
                winner = "none"
                settings_button = store_grid()
