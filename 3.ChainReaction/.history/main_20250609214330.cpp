...
void explode(vector<vector<Cell>>& grid,char color,int row, int column) {
    grid[row][column].set_orb_num(0);
    grid[row][column].set_color('w');
    grid[row+1][column].set_color(color);
    grid[row-1][column].set_color(color);
    grid[row][column+1].set_color(color);
    grid[row][column-1].set_color(color);
}

void inc_cell_orbs(vector<vector<Cell>>& grid, char user_color)
...
int userMove(vector<vector<Cell>>& grid,char user_color,int row,int column) {
    ...
    if(target.get_orb_num() == target.get_critical_mass()) {
        explode(grid,user_color,row,column);
    }
    else{
