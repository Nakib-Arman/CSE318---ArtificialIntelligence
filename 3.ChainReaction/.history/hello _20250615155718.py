...
def update_details():
    global rows,cols,cell_x,cell_y,screen_x,screen_y,grid_x,grid_y,cell_surface
    cell_x, cell_y = (screen_x-700) // cols, (screen_y-100) // rows
    grid_x = (screen_x-cols*cell_x) // 2
    grid_y = (screen_y-rows*cell_y) // 2
    cell_surface = pygame.Surface((cell_x,cell_y),pygame.SRCALPHA)
...
def store_grid():
    global rows,cols,colors,counts
...
def load_ui():
    global rows,cols,cell_surface,grid_x,grid_y,cell
    # background image
    screen.fill((0, 0, 0))
