...
Node* makeDecisionTree(vector<vector<string>>& dataset, vector<pair<string,false>& attributes, vector<unordered_set<string>>& values)
{   
    float max_ig = 0;
    int max_i=0;
    ...
    Node* root = new Node(attributes[max_i]);
    for(string val: values[values.size()-1]){
        for(int i=0;i<dataset.size();i++){
