...
{   
    for(string val:values[1]) cout<<val;
    ...
    for(string val: values[max_i]){
        ...
        if(is_digit(val)){
            ...
            if(temp_dataset1.size() == 0) {
                ...
                // for(string val: values[1]) cout<<val<<endl;
...
int main()
{
    ...
    vector<vector<float>> rainmeric(rain.size());
    ...
    for(int i=0;i<dataset.size();i++){
        if(dataset[i][0] == "Rain"){
            rain.push_back(dataset[i]);
            // rainmeric[i].push_back(stof())
        }
    }

    // cout<<getInformationGain(rain,values[values.size()-1],1,values[1],numeric_values[1])
    ...
    //     cout<<endl;
    // }
    Node* root = makeDecisionTree(dataset,attributes,values,numeric_values);
    // for(int i=0;i<dataset.size();i++){
