...
vector<pair<float,bool>> calculateMaxThreshold(vector<vector<string>>& dataset,vector<unordered_set<string>>& values,unordered_map<string,int>& decisions,vector<pair<float,bool>>& used)
{
    vector<pair<float,bool>> temp_used = used;
    for(int i=0;i<values.size();i++){
        ...
        temp_used[i].first = max_threshold;
    }
    cout<<temp_used[1].first<<endl;
    return temp_used;
}
    ...
    for(string val: values[max_i]){
        ...
        if(is_digit(val)){
            ...
            if(temp_dataset1.size() == 0) {
                ...
                vector<pair<float,bool>> temp_used = calculateMaxThreshold(temp_dataset1,temp_values1,temp_decisions1,used);
                // cout<<"after"<<endl;
                // for(int i=0;i<used.size();i++){
                //     cout<<used[i].first<<endl;
                // }
                ...
                // }
                Node* child1 = makeDecisionTree(temp_dataset1,temp_values1,temp_decisions1,temp_used);
            ...
            if(temp_dataset2.size() == 0){
                ...
                // }
                string curr_thresh = to_string(used[max_i].first);
                vector<pair<float,bool>> temp_used = calculateMaxThreshold(temp_dataset2,temp_values2,temp_decisions2,used);
                Node* child2 = makeDecisionTree(temp_dataset2,temp_values2,temp_decisions2,temp_used);
            ...
            //     cout<<used[i].first<<" "<<used[i].second<<endl;
            // }
            vector<pair<float,bool>> temp_used = calculateMaxThreshold(temp_dataset,temp_values,temp_decisions,used);
            // for(int k=0;k<used.size();k++){
            //     cout<<used[k].first<<" "<<used[k].second<<endl;
            // }
            Node* child = makeDecisionTree(temp_dataset,temp_values,temp_decisions,used);
...
int main()
{
    ...
    //     cout<<used[i].first<<endl;
    // }
    makeDecisionTree(dataset,values,decisions,used);
