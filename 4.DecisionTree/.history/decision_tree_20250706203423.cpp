    ...
    for(string val: values[max_i]){
        if(is_digit(val)){
            ...
            for(int i=0;i<dataset.size();i++){
                if(stof(dataset[i][max_i]) < stof(val)){
                    ...
                    for(int j=0;j<numeric_values.size();j++){
                        temp_numeric_values1[j][i] = numeric_values[j][i];
                    }
                }
                else{
                    temp_dataset2.push_back(dataset[i]);
                    for(int j=0;j<numeric_values.size();j++){
                        temp_numeric_values2[j][i] = numeric_values[j][i];
                    }
                }
            }
            for(int i=0;i<temp_numeric_values1.size();i++){
                for(int j=0;j<temp_numeric_values1[0].size();j++){
            ...
            if(temp_dataset1.size() == 0) {
                ...
                Node* child1 = makeDecisionTree(temp_dataset1,attributes,values,numeric_values);
