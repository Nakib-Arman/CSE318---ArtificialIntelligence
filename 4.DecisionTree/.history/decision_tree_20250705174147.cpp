...
Node* makeDecisionTree(vector<vector<string>>& dataset, vector<pair<string,bool>>& attributes, vector<unordered_set<string>>& values)
{   
    float max_ig = 0;
    int max_i=0;
    ...
    for(int i=0;i<attributes.size();i++){
        if(attributes[i].second == false) check = false;
    }
    if(check) return new Node("true");
    ...
    cout<<attributes[max_i].first<<attributes<<" Selected"<<endl;
