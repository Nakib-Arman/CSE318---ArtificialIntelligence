...
void makeDecisionTree(vector<vector<string>>& dataset, vector<string>& attributes, vector<unordered_set<string>>& values)
{   
    float max_ig = 0;
    for (int i=0;i<attributes.size()-1;i++){
        float ig = getInformationGain(dataset,values[values.size()-1],i,)
    }
}
