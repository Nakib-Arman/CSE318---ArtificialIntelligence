...
float getInformationGain(vector<vector<string>>& dataset, unordered_set<string>& decisions, int attribute_num, unordered_set<string>& values,float threshold)
{
        ...
        if(is_digit(val)){
            ...
            if(temp_dataset1.size() == 0) {
                ...
                for(int i=0;i<temp_numeric_values1.size();i++){
                    ...
                    for(int j=0;j<temp_dataset1.size();j++){
                        values[i].insert(to_string(temp_numeric_values1[i][j]));
                        float ig = getInformationGain(temp_dataset1,values[values.size()-1],i,values[i]);
                        if(ig>max_ig){
                            max_ig = ig;
                            max_index = j;
                        }
                        values[i].erase(to_string(temp_numeric_values1[i][j]));
                    }
            ...
            if(temp_dataset2.size() == 0){
                ...
                for(int i=0;i<temp_numeric_values2.size();i++){
                    ...
                    for(int j=0;j<temp_dataset2.size();j++){
                        values[i].insert(to_string(temp_numeric_values2[i][j]));
                        float ig = getInformationGain(temp_dataset2,values[values.size()-1],i,values[i]);
                        if(ig>max_ig){
                            max_ig = ig;
                            max_index = j;
                        }
                        values[i].erase(to_string(temp_numeric_values2[i][j]));
                    }
            ...
            for(int i=0;i<temp_numeric_values.size();i++){
                ...
                for(int j=0;j<temp_dataset.size();j++){
                    values[i].insert(to_string(temp_numeric_values[i][j]));
                    float ig = getInformationGain(temp_dataset,values[values.size()-1],i,values[i]);
                    if(ig>max_ig){
                        max_ig = ig;
                        max_index = j;
                    }
                    values[i].erase(to_string(temp_numeric_values[i][j]));
                }
