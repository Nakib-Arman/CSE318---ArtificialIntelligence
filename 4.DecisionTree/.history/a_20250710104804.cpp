    ...
    for(string val: values[max_i]){
        ...
        if(is_digit(val)){
            ...
            if(temp_dataset1.size() == 0) {
                ...
                vector<pair<float,boot>> temp_used = calculateMaxThreshold(temp_dataset1,temp_values1,temp_decisions1,used);
                // cout<<"after"<<endl;
                // for(int i=0;i<used.size();i++){
                //     cout<<used[i].first<<endl;
                // }
                ...
                // }
                Node* child1 = makeDecisionTree(temp_dataset1,temp_values1,temp_decisions1,used);
